...
func build(p problem) {
	if GoKit.Exist(p.path()) {
		log.Panicf("第 %d 题的文件夹已经存在，请 **移除** %s 文件夹后，再尝试。", p.ID, p.path())
	}
	...
	if err != nil {
		log.Panicf("无法创建目录，%s ：%s", p.path(), err)
	}
...
`
	...
	filename := fmt.Sprintf("%s/%s.go", p.path(), p.TitleSlug)
...
`
	...
	filename := fmt.Sprintf("%s/%s_test.go", p.path(), p.TitleSlug)
